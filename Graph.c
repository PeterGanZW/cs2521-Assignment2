...
Graph newGraph(int noNodes){
    ...
    for(int i =0; i<noNodes; i++){                          // set all the node pointer in the list to be NULL
        graph->List[i] = NULL;
    }
...
AdjList inIncident(Graph g, Vertex v){
    ...
    for (int i =0; i< g->noNodes; i++){          // for every adjacent list
        if (i == v) continue;
...
void  showGraph(Graph g){
    ...
    for(int i =0; i<g->noNodes; i++){            // for every adjacency list
        adjListNode* curr = g->List[i];          
        printf("%d ----->", i);
        while (curr != NULL){
